...
def oneRun(log_dir, output_dir_experiment, **params):
    ...
    def margin_loss_fn(logits, labels, margin=0.5):
        device = logits.device  # 获取 logits 的设备
        labels_one_hot = F.one_hot(labels, num_classes=logits.size(-1)).float().to(device)  # 确保 labels_one_hot 在同一个设备上

        positive_mask = labels_one_hot
        negative_mask = 1 - positive_mask

        positive_loss = positive_mask * torch.clamp(margin - logits, min=0)
        negative_loss = negative_mask * torch.clamp(logits - margin, min=0)
            ...
            if load_mode == 'semi':
                for _ in range(ul_ratio):
                    ...
                    for i in range(num_nets):
                        ...
                        # 边界损失计算
                        margin_loss = margin_loss_fn(outs_x_ulb_s_nets[i], batch_unlabel['label'])
                        ...
                        total_unsup_loss = weight_u_loss * (unsup_loss + margin_loss)
                    ...
                    # Add contrastive loss to the training process
                    # for i in range(num_nets):
                    #     netgroup.update_net(netgroup.nets[i], netgroup.optimizers[i], contrastive_loss)
                    if ema_mode:
                        netgroup.update_ema()
