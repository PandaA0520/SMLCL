...
class MyCollator(object):
    ...
    def __call__(self, batch):
        ...
        if sents_aug1 is not None:
            ...
            tokenized_aug1 = self.tokenizer(sents_aug1, padding=True, truncation='longest_first', max_length=255,
                                            return_tensors='pt')
        else:
        ...
        if sents_aug2 is not None:
            ...
            tokenized_aug2 = self.tokenizer(sents_aug2, padding=True, truncation='longest_first', max_length=255,
                                            return_tensors='pt')
        else:
            tokenized_aug2 = None
...
def get_dataloader(data_path, n_labeled_per_class, bs, load_mode='semi'):
    ...
    train_l_df, train_u_df = train_df.iloc[train_labeled_idxs].reset_index(drop=True), train_df.iloc[
        train_unlabeled_idxs].reset_index(drop=True)
    ...
    if load_mode == 'semi':
        if 'yahoo' in data_path:
            ...
            bt_l_df, bt_u_df = bt_df.iloc[train_labeled_idxs].reset_index(drop=True), bt_df.iloc[
                train_unlabeled_idxs].reset_index(drop=True)
            train_dataset_l = SEMIDataset(train_l_df['content'].to_list(), train_l_df['synonym_aug'].to_list(),
                                          bt_l_df['back_translation'], labels=train_l_df['label'].to_list())
            train_dataset_u = SEMIDataset(train_u_df['content'].to_list(), train_u_df['synonym_aug'].to_list(),
                                          bt_u_df['back_translation'], labels=train_u_df['label'].to_list())
        else:
            train_dataset_l = SEMIDataset(train_l_df['content'].to_list(), train_l_df['synonym_aug'].to_list(),
                                          train_l_df['back_translation'], labels=train_l_df['label'].to_list())
            train_dataset_u = SEMIDataset(train_u_df['content'].to_list(), train_u_df['synonym_aug'].to_list(),
                                          train_u_df['back_translation'], labels=train_u_df['label'].to_list())
        train_loader_u = DataLoader(dataset=train_dataset_u, batch_size=bs, shuffle=True,
...
if __name__ == '__main__':
    ...
    for data_path in data_path_list:
        for load_mode in load_mode_list:
            ...
            train_loader_l, train_loader_u, dev_loader, test_loader, num_class = get_dataloader(data_path,
                                                                                                n_labeled_per_class, bs,
                                                                                                load_mode)
