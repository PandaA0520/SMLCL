...
class NetGroup(nn.Module):
    ...
    def init_net(self, net_arch):
        if net_arch == 'bert-base-uncased':
            ...
            net = AutoModelForSequenceClassification.from_pretrained(model_path, num_labels = self.n_classes, output_hidden_states=True)
    ...
    def extract_features(self, x):
        ...
        for i in range(self.num_nets):
            features.append(self.nets[i].extract_features(x))
        return torch.stack(features)  # [num_nets, batch_size, feature_size]
